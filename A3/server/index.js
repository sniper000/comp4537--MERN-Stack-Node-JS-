  ...
  asyncWrapper(async (req, res, next) => {
    ...
    try {
      if (req.query["appid"].length > 1) {
        ...
        const userStoredJWTTokenFromDB = await userModel.findOne({
          jwt: appid,
        });
        if (
          userStoredJWTTokenFromDB === undefined ||
          userStoredJWTTokenFromDB === null
        ) {
          throw new PokemonBadRequest(
            "Error! JWT Token does not match JWT token in DB"
          );
        }
        if (userStoredJWTTokenFromDB.jwt === req.query["appid"]) {
          ...
          throw new PokemonNotFoundError(
            "JWT token does not match JWT token in DB"
          );
        }
      } else {
        throw new PokemonNotFoundError(
          "Missing JWT Token from user as API Key"
        );
      }
    } catch (err) {
      next(err);
    ...
    try {
      if (req.query["appid"].length > 1) {
        ...
        if (userStoredJWTTokenFromDB.jwt === req.query["appid"]) {
          ...
          if (
            userStoredJWTTokenFromDB.admin === false ||
            userStoredJWTTokenFromDB.admin !== true
          ) {
            throw new PokemonBadRequest(
              "User's role is not authorize to access pokemon image. Admin Role Privilege is required to access pokemon image."
            );
          }
          if (userStoredJWTTokenFromDB.admin === true) {
            await pokeModel
              ...
              .then((doc) => {
                ...
                var pokemonImage = {
                  url: `https://raw.githubusercontent.com/fanzeyi/pokemon.json/master/images/${req.query.id}.png`,
                };
    ...
    try {
      if (req.query["appid"].length > 1) {
        ...
        if (userStoredJWTTokenFromDB.jwt === req.query["appid"]) {
          ...
          if (userStoredJWTTokenFromDB.admin === true) {
            try {
              const docs = await pokeModel.findOneAndRemove({
                id: req.query.id,
              });
              if (docs)
                res.json({
        ...
        if (userStoredJWTTokenFromDB.jwt === req.query["appid"]) {
          ...
          if (userStoredJWTTokenFromDB.admin === true) {
            try {
              ...
              const doc = await pokeModel.findOneAndUpdate(
                selection,
                update,
                options
              );
...
app.get("/report", (req, res) => {
  ...
  // res.json({
  //   data: "admin report requested placeholder",
  //   route: `Table ${req.query}`,
  // });
  console.log(req.query.id);
  if (req.query["id"] === "1") {
    res.json({
